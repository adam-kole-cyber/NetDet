...
static inline void sync_display_limit(device_table_view *buffer) {
	buffer->view.visible = (atomic_load(&buffer->view.viewport_capacity) < buffer->data->size)
							   ? atomic_load(&buffer->view.viewport_capacity)
							   : buffer->data->size;
}
...
void draw_table_header(WINDOW *window) {
	...
	mvwprintw(window, 1, 2, "%-*s%-*s%-*s%-*s%*s", current_layout.col_width, "MAC", current_layout.col_width, "IP",
			  current_layout.col_width, "802.1ad", (current_layout.col_width / 2), "802.1Q",
			  ((current_layout.col_width / 2) + current_layout.col_width_remainder), "Last seen");
